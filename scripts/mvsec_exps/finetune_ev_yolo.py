"""Finetuning script for the event based YOLO.
...
#  pretrained_model=copy.deepcopy(pretrain_model.darknet)).to(DEVICE)
...
if lstm_frontend is not None:
    optimizerG = Adam(
        [{"params": ev_yolo.parameters()},
         {"params": lstm_frontend.parameters()}],
        lr=1e-4)
else:
...
for epoch_i in range(args.num_epochs):
    ...
    torch.save(
        {"epoch": epoch_i,
         ...
         "r1_dict": lstm_frontend.state_dict() if args.r1 is True else None},
        checkpoint_path)
