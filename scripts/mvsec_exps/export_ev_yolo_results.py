...
from evtrans.networks import ConvLSTMFrontend
...
def get_recurrent_initial_state(img_size, r1, r2, device):
    """Generate initial state for recurrent frontend."""
    # set up initial state
    if args.r1 is True:
        recurrent_hid = 1
    elif args.r2 is True:
        recurrent_hid = 32
...
#  # number of inputs in middle net
#  parser.add_argument("--num_eval_inputs", type=int)

# with extra recurrent layer
parser.add_argument("--r1", action="store_true")
# replace first layer as recurrent layer
...
if args.r1 is True and args.r2 is False:
    lstm_frontend = ConvLSTMFrontend(
        img_size=args.img_size, input_dim=1,
        hidden_dim=1, kernel_size=(3, 3), bias=True).to(DEVICE)
    lstm_frontend.load_state_dict(checkpoint["r1_dict"])
    lstm_frontend.eval()
    print("R1 model is enabled")
else:
    lstm_frontend = None
...
if args.r2 is True and args.r1 is False:
    #  lstm_frontend = ConvLSTMFrontend(
    #      img_size=args.img_size, input_dim=1,
    #      hidden_dim=32, kernel_size=(3, 3), bias=True).to(DEVICE)
    #  lstm_frontend.load_state_dict(checkpoint["r2_dict"])
    #  lstm_frontend.eval()

    print("Placeholder, not using it.")
    print("R2 mode is enabled")
...
classes = load_classes("../res/coco.names")

with open("../res/ilsvrc.names") as f:
...
if args.r1 is True:
    h_state, c_state = get_recurrent_initial_state(
        args.img_size, args.r1, args.r2, DEVICE)

#  files_list = files_list[2000:]
...
for file_idx in range(len(files_list)):
    ...
    with torch.no_grad():
        ...
        if lstm_frontend is not None:
            h_state, c_state = lstm_frontend(candidate, h_state, c_state)
            pre_detections = ev_yolo(
                torch.cat([h_state, h_state, h_state], dim=1))
        else:
    ...
    if args.vis is True:
        ...
        #  ax1 = plt.subplot(337)
        #  h_state_data = h_state.detach().cpu().data.numpy()
        #  h_state_data = (h_state_data-h_state_data.min())/(
        #      h_state_data.max()-h_state_data.min())
        #  #  sns.heatmap(h_state_data[0, 0],
        #  #              cmap="gray", square=True)
        #  plt.imshow(h_state_data[0, 0], cmap="gray")
        #  plt.title("LSTM State")
        #  plt.axis("off")
        #  for box_id in range(detections.shape[0]):
        #      try:
        #          rect = patches.Rectangle(
